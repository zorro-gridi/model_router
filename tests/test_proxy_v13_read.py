...
测试目标：
  ...
  3. 两者都有 → 新格式胜出
  4. 都没有 → None
  5. 新格式 JSON 损坏 → fallback 旧文件
  6. 新格式无 decision 字段 → 返回空 dict（不报错）
"""
...
# ── 场景 6: 新格式无 decision 字段 → 空 dict 不报错 ───────────────────────
