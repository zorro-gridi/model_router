...
def _log_classify_result(result: dict) -> None:
    ...
    try:
        _log.info(
            ...
            "stage=%s task_field=%s(p=%.2f) reasoning=%s",
            result.get("pattern"),
            ...
            result.get("stage"),
            result.get("task_field"),
            result.get("task_field_confidence", 0.0),
...
Analyze the user's request and classify it along FOUR dimensions.
...
## Dimension 4 — task_field (业务领域，用于 statusline 展示，不参与路由决策):
任务所属业务领域。必须输出下列 5 个枚举值之一，无法判断时输出 "unknown"。
- "frontend": 前端领域 — UI 组件、页面、样式、交互、响应式、React/Vue/小程序/H5/HTML/CSS/JS/TS 等前端技术栈
- "backend":  后端领域 — 接口、API、数据库、服务端逻辑、Python/Go/Java/Node 服务、ORM、消息队列、缓存等
- "ops":      运维领域 — CI/CD、部署、容器、K8s、监控告警、日志、脚本、cron、定时任务、nginx 配置等
- "product":  产品领域 — PRD、需求分析、产品方案、用户故事、竞品分析、流程梳理、原型等
- "unknown":  无法判定（如通用对话、闲聊、提问而非实施）
    ...
    Returns:
        {
            ...
            "task_field": str,               # V1.4 新增：业务领域
                                              #   frontend | backend | ops | product | unknown
                                              #   不参与路由决策，仅 statusline 第三行展示
            "task_field_confidence": float,  # 业务领域置信度 0~1
...
# 业务领域枚举（task_field）—— 用于 statusline 展示，**不参与路由决策**
# 5 个合法值：frontend / backend / ops / product / unknown
# 用户自定义配置可扩展，但默认走这套枚举
VALID_TASK_FIELDS: frozenset[str] = frozenset(
    {"frontend", "backend", "ops", "product", "unknown"}
)
# 用户自定义 task_field 白名单：可通过 hooks/model_router/config/task_fields.yaml
# 注入额外合法值。validate_and_normalize 仍以 VALID_TASK_FIELDS 为主校验，
# 若 LLM 返回自定义值，落到兜底 "unknown"，避免破坏 schema 形状契约。
# 本轮 V1.4 暂未实现 YAML 加载，仅保留枚举与注释占位。
...
def _validate_and_normalize(raw: dict, prompt: str) -> dict:
    ...
    # ── task_field（V1.4 新增：业务领域，statusline 展示用）──
    # 5 个合法枚举：frontend / backend / ops / product / unknown
    # 非法值 / 缺失值 → 兜底 "unknown"（保留 schema 形状契约）
    task_field = str(raw.get("task_field", "")).strip().lower()
    if task_field not in VALID_TASK_FIELDS:
        task_field = "unknown"

    # ── task_field_confidence ──
    try:
        task_field_confidence = float(raw.get("task_field_confidence", 0.5))
    except (ValueError, TypeError):
        task_field_confidence = 0.5
    task_field_confidence = max(0.0, min(1.0, round(task_field_confidence, 2)))
    ...
    return {
        ...
        "task_field_confidence":  task_field_confidence,
        "reasoning":              reasoning,
    }
